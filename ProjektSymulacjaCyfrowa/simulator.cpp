...
void Simulator::GetStats()
{
    ...
    vector<size_t> *packet_delay_times = wireless_network_->GetPacketDelayTimes(); 
    vector<size_t>* packet_waiting_times = wireless_network_->GetPacketWaitingTime();
    double sum = 0.0;
    size_t sim_time;

    cout << "\n";
    for (int i = 0; i < wireless_network_->GetTranmitters()->size(); i++)
    {
        double error_tx_average = 0.0;
        double succes = wireless_network_->GetTranmitters()->at(i)->GetNrofSucces();
        double fail = wireless_network_->GetTranmitters()->at(i)->GetNrOfFail();
        if ((wireless_network_->GetTranmitters()->at(i)->GetNrOfRetransmissionSucces()->size())>0)
        {
            for ( j = 0; j < wireless_network_->GetTranmitters()->at(i)->GetNrOfRetransmissionSucces()->size(); j++)
            {
                tmp_retransmissions = wireless_network_->GetTranmitters()->at(i)->GetNrOfRetransmissionSucces()->at(j);
    ...
    sim_time = (clock_ - wireless_network_->GetStartSimTime())/10;
...
void Simulator::M2(int time)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        int cgpk = wireless_network_->GetTranmitters()->at(i)->GetCgpkGenerator()->RndExp(lambda) * 10;
        wireless_network_->cgpk_numbers.push_back(cgpk);
        event_list.push(new GeneratePacketEvent(cgpk, wireless_network_, &event_list, i));
    }
    //----------------------------------------------------------------------------------------------------
